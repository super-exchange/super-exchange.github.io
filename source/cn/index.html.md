...
欢迎使用Super-exchange开发者文档，此文档是Super-exchange API的唯一官方文档。
...
Super-exchange API提供的服务会在此持续更新，请大家及时关注。
...
[您可以点击创建APIKey]: https://www.super-exchange.co/#/user/bindAuth/ManagementAPI
...
## 接口类型
Super-exchange为用户提供两种接口，您可根据自己的使用场景和偏好来选择适合的方式进行查询行情、交易。
...
一个合法的请求由以下几部分组成：
- 方法请求地址：即访问服务器地址api.super-exchange.co，比如https://api.super-exchange.co/api/trade/order/place
- 必须和可选参数。
...
#### 目前支持合约<一>现货划转, API域名地址 `https://futures.api.super-exchange.co/api`  调用支持ApiKey
...
#### 新接口的 API域名地址 `https://api.super-exchange.co`  调用支持ApiKey
...
#### 普通用户的账号同一时间只允许持有 50 笔活动委托，做市账号不受此限制。如果您是做市商但有账号未添加做市账号权限，请联系 Super-exchange 交付部门。
